...
const FoodItem = () => {
  ...
  return (
    ...
    <div className='grid grid-cols-1 gap-4 mt-10 mb-5 lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 place-items-center lg:mx-8 md:mx-4 sm:mx-2'>
      {
        FoodData.map((item) => {
          return <FoodCard key={item.id} id={item.id} image={item.img} name={item.name} price={item.price} desc={item.desc} category={item.category} rating={item.rating} handleToast={handleSuccessToast}/>
        })
      }
    </div>
    </>
